...
**Status:** Phases 1-7(1+2) + phase 8 (slices 1-5) + phase 6.3b + 6.3c(part 1) shipped 2026-05-03. Phase 7.3 BLOCKED behind 6.3c part 2 + locations_stage retire (audit at [docs/harmony-7-3-audit.md](harmony-7-3-audit.md)). Phase 8 = curator-on-canonical-tables migration (see `~/.claude/projects/C--Users-beach/memory/project_curator_on_canonical_tables.md`). New curator surface is `admin/beach-editor-gold.html`; legacy `location-editor.html` carries a deprecation banner pointing at it.
...
| 6.3b | Operator-keyed populator: `populate_from_park_operators_gold`. Joins via `csp_parks` × `park_operators` (catches "CDPR state park leased to city/county" cases per `project_state_park_operators.md`). 4 evidence rows emitted (Santa Monica SB → City of Santa Monica; Corona del Mar SB + Little CdM → City of Newport Beach; Leucadia SB → City of Encinitas). Idempotent. | shipped 2026-05-03 |
| 6.3c (part 1) | `populate_from_research_gold`. Reads policy_research_extractions (arena_group_id IS gold_fid). Maps `origin` → source name (v2_dog_policy_old → old_school_llm, v2_dog_policy_v2 → research, manual → manual). 1,102 rows emitted across 503 distinct beaches: old_school_llm has 444 dogs + 430 practical; research has 228 dogs (v2 was dog-focused, no practical). | shipped 2026-05-03 |
| 6.3c (part 2) | `populate_from_park_url_gold` with the buffer-rescued attribution path that joins us_beach_points + beach_cpad_candidates. Both join tables are still in legacy fid namespace; needs gold-spine equivalents before the populator can be ported cleanly. | **STILL DEFERRED 2026-05-03** — needs us_beach_points_gold + beach_cpad_candidates_gold |
