"""Phase 2 backfill: re-fetch geometry from OSM Overpass for osm_landing
rows whose geom_full is NULL and whose tags include `note` / `description`
/ `note:en` / `description:en`. After phase 1 (pure-SQL hydration from
lat/lon + raw_geom JSONB), 541 rows in this cohort still need fetching.

Triggered by: Franz, 2026-05-11. Path B of the OSM-note-harvesting plan.
The 574-row gap surfaced when we tried to surface OSM `note` content
(crowd-voice nuance) for the description generator and found that 68%
of crowd-tagged content has NULL geom_full so spatial joins miss it.

Batches by OSM type (way/node/relation), 80 IDs per batch, ~10s between
batches (Overpass politeness). Idempotent — re-run is safe.
"""
...
BATCH = 80
RATE_SLEEP = 8.0  # seconds between batches
        ...
        cur.execute("""
          select type, id from public.osm_landing
           where is_active and geom_full is null
             and (tags ? 'note' or tags ? 'description'
                  or tags ? 'note:en' or tags ? 'description:en')
        """)
        return cur.fetchall()
...
def overpass_fetch(kind: str, ids: list[int]) -> list[dict]:
    """Fetch a batch of OSM elements with full geometry."""
...
def element_to_geojson(e: dict) -> dict | None:
    """Turn an Overpass element into a GeoJSON geometry."""
    t = e.get("type")
    if t == "node":
    ...
    if t == "way":
        ...
        # Closed way → Polygon, else LineString
        if len(coords) >= 4 and coords[0] == coords[-1]:
            return {"type": "Polygon", "coordinates": [coords]}
        return {"type": "LineString", "coordinates": coords}
    if t == "relation":
        # Build MultiPolygon from outer rings; ignore inner ring nesting.
        outers = []
...
def upsert_geom(rows: list[tuple]):
    """rows: list of (kind, id, geojson_dict)"""
    if not rows:
        return 0
    with psycopg2.connect(**PG) as c, c.cursor() as cur:
...
def main():
    ...
    print(f"targets: {len(targets)} note/desc rows with NULL geom_full", flush=True)
    if not targets:
        return
    ...
    for kind, oid in targets:
        if kind in by_kind:
            ...
            print(f"  unexpected type: {kind}/{oid}", file=sys.stderr)
        ...
        cur.execute("""select count(*) filter (where geom_full is null) still_null,
                         ...
                         from public.osm_landing where is_active
                          and (tags ? 'note' or tags ? 'description'
                            or tags ? 'note:en' or tags ? 'description:en')""")
        r = cur.fetchone()
        print(f"note/desc cohort: {r[1]-r[0]}/{r[1]} now have geom_full ({r[0]} still null)")
