"""oh_overnight_observer.py — polls the OH Dagster run, writes morning report.
...
Output: tmp/oh_morning_report.md
"""
...
RUN_ID  = 'ed2bf529-f76b-4036-913a-62f1332b3799'
REPORT  = REPO / 'tmp' / 'oh_morning_report.md'
...
def db_metrics():
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        cur.execute("""
          SELECT
            (SELECT count(*) FROM public.arena WHERE county_fips LIKE '39%' AND is_active) AS arena_active,
            (SELECT count(*) FROM public.beaches_gold WHERE state='OH' AND is_active) AS gold,
            (SELECT count(*) FROM public.beaches_gold WHERE state='OH' AND is_active AND scoring_tier='daily') AS daily,
            (SELECT count(*) FROM public.beaches_gold WHERE state='OH' AND is_active AND scoring_tier='hourly') AS hourly,
            (SELECT count(*) FROM public.beaches_gold WHERE state='OH' AND is_active AND scoring_tier='none') AS tier_none
        """)
        ...
        cur.execute("""
          SELECT bdp.dogs_allowed, count(*) AS n
           ...
           WHERE g.state='OH' AND g.is_active AND g.scoring_tier IN ('daily','hourly')
        ...
        cur.execute("""
          SELECT count(*) AS extractions,
           ...
           WHERE o.state_code='OH' AND ope.extracted_at > now() - interval '12 hours'
        """)
...
def write_report(run_data, fail_causes, metrics):
    ...
    md.append(f'- Arena rows (OH county_fips 39*): **{f["arena_active"]}**')
