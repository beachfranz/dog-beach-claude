...
2. From `verified_physical_features`, drop UP TO THREE features naturally ("backed by coastal bluffs", "at the mouth of Aliso Creek", "a jetty marks the south end"). Two or three depending on how much character is grounded — three when each adds distinct geography. Never enumerate four; that's a list, not character.
2a. **Walkable spaces — dogs love walks.** When `zones.sections.trails` is set (named trail with its own leash rule) OR `verified_physical_features` has a named `cycleway` / `footway` / `path` / `boardwalk` / `pedestrian` entry, surface it as a leashed-walk extension — "the Huntington Beach Trail runs alongside if you want a leashed walk", "a boardwalk extends north from the parking lot", "a paved pedestrian path runs the length of the bluffs". This is high-value to dog owners: adjacent walkable space lets a beach visit become a longer outing. Treat it as ONE dedicated mention, separate from bullet #2's terrain cap. Skip if unnamed or if the beach itself is the only walkable space (no nearby path features).
