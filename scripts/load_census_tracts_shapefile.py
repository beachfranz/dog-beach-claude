"""load_census_tracts_shapefile.py

Loads a single TIGER/Line 2024 census-tract shapefile (one state) into
public.census_tracts via the load_census_tracts_batch JSONB function.

Reprojects NAD83 (EPSG:4269) -> WGS84 (EPSG:4326) per project CRS
convention. Same direct-SQL batch pattern as load_places_shapefile.py.

Called by scripts/one_off/bulk_load_census_tracts.py per state. Can
also be run standalone:

  python scripts/load_census_tracts_shapefile.py supabase/.temp/tiger/tl_2024_06_tract
"""
...
SQL_DIR = PROJECT_ROOT / 'supabase' / '.temp' / 'tracts_sql'
BATCH_SIZE = 25  # tracts per batch — keep small because polygons are bigger than places

FIELDS = ["STATEFP", "COUNTYFP", "TRACTCE", "GEOID", "NAME", "NAMELSAD",
...
def main():
    if len(sys.argv) < 2:
        print('usage: load_census_tracts_shapefile.py <shp_basename_no_ext>')
        sys.exit(2)
    ...
    for b, batch in enumerate(chunk(features, BATCH_SIZE)):
        ...
        sql = f"select load_census_tracts_batch('{json_literal}'::jsonb);\n"
    ...
    for b in range(n_batches):
        ...
        if rc == 0:
            ...
            fail += 1
            print(f"  batch {b} FAILED: {(err or '')[:200].strip()}", file=sys.stderr)
