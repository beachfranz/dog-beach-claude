"""md_overnight_observer.py — polls the MD Dagster run, writes morning report.
...
Output: tmp/md_morning_report.md
"""
...
def db_metrics():
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        cur.execute("""
          SELECT
            (SELECT count(*) FROM public.arena WHERE county_fips LIKE '24%' AND is_active) AS arena_active,
            (SELECT count(*) FROM public.beaches_gold WHERE state='MD' AND is_active) AS gold,
            (SELECT count(*) FROM public.beaches_gold WHERE state='MD' AND is_active AND scoring_tier='daily') AS daily,
            (SELECT count(*) FROM public.beaches_gold WHERE state='MD' AND is_active AND scoring_tier='hourly') AS hourly,
            (SELECT count(*) FROM public.beaches_gold WHERE state='MD' AND is_active AND scoring_tier='none') AS tier_none
        """)
        ...
        cur.execute("""
          SELECT bdp.dogs_allowed, count(*) AS n
           ...
           WHERE g.state='MD' AND g.is_active AND g.scoring_tier IN ('daily','hourly')
        ...
        cur.execute("""
          SELECT count(*) AS extractions,
           ...
           WHERE o.state_code='MD' AND ope.extracted_at > now() - interval '12 hours'
        """)
...
def write_report(run_data, fail_causes, metrics):
    ...
    md.append(f'- Arena rows (MD county_fips 24*): **{f["arena_active"]}**')
