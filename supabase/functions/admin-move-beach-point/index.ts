// admin-move-beach-point/index.ts
// Move a beaches_gold catalog point to new lat/lon. Used by the
// auto-polygon-review UI when a curator drags the marker. Updates
// lat, lon, and geom in sync via the move_beach_gold_point RPC.
// Body: { fid: number, lat: number, lon: number }
...
Deno.serve(async (req: Request) => {
  ...
  const { data, error } = await supabase.rpc("move_beach_gold_point",
  ...
  if (pair) {
    await logAdminWrite(supabase, {
      functionName: "admin-move-beach-point",
      action:       "update",
      req,
      before:       pair.before,
      after:        { ...pair.after, __resolution_mode: "point_dragged" },
      success:      true,
    });
  }
