...
Deno.serve(async (req: Request) => {
  ...
  const { data: rows, error } = await supabase
    ...
    .select("id, display_name, governing_body_source")
    .eq("governing_body_source", "county_default");
  if (error) return json({ error: error.message }, 500);
  if (!rows?.length) return json({ processed: 0, marked: 0 });
  ...
  if (body.dry_run) {
    return json({ dry_run: true, processed: rows.length, matched: hits.length, preview: hits });
  }
  ...
  return json({ processed: rows.length, matched: hits.length, marked, errors: writeErrors });
});
