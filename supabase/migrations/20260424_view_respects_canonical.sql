-- beach_access_source view now respects canonical_* override (2026-04-24)
--
-- When us_beach_points.canonical_source is set, the view returns the bucket
-- derived from that explicit authority record rather than from spatial
-- inference. Admin-pinned associations win. Falls back to the existing
-- dual-pass (nearest-named + nearest-overall) logic when canonical_* is null.
...
tribal_best as (
  select distinct on (b.fid)
    b.fid, t.lar_name as match_name,
...
plz_best as (
  select distinct on (b.fid)
    b.fid, z.name as match_name, z.reason as plz_reason
  from public.us_beach_points b
...
-- Resolve admin-pinned canonical refs to a concrete record + distance
canonical_resolved as (
  select
    b.fid,
    b.canonical_source as source,
    b.canonical_name   as match_name,
    b.canonical_county as county,
    -- Distance to the pinned record (0 for in-polygon cases, else real)
    case b.canonical_source
      when 'cpad' then (
        select round(ST_Distance(c.geom::geography, b.geom::geography)::numeric, 0)::int
        from public.cpad_units c
        where c.unit_name = b.canonical_name and trim(c.county) = trim(b.canonical_county)
        order by c.geom <-> b.geom limit 1
      )
      when 'ccc' then (
        select round(ST_Distance(c.geom::geography, b.geom::geography)::numeric, 0)::int
        from public.ccc_access_points c
        where c.name = b.canonical_name and trim(c.county) = trim(b.canonical_county)
        order by c.geom <-> b.geom limit 1
      )
      when 'csp' then (
        select round(ST_Distance(s.geom::geography, b.geom::geography)::numeric, 0)::int
        from public.csp_parks s
        where s.unit_name = b.canonical_name
        order by s.geom <-> b.geom limit 1
      )
    end as dist_m,
    -- Per-source metadata for the passthrough columns
    case when b.canonical_source = 'cpad' then (
      select c.access_typ from public.cpad_units c
      where c.unit_name = b.canonical_name and trim(c.county) = trim(b.canonical_county) limit 1
    ) end as cpad_access_typ,
    case when b.canonical_source = 'cpad' then (
      select c.agncy_lev from public.cpad_units c
      where c.unit_name = b.canonical_name and trim(c.county) = trim(b.canonical_county) limit 1
    ) end as cpad_agncy_lev,
    case when b.canonical_source = 'ccc' then (
      select c.open_to_public from public.ccc_access_points c
      where c.name = b.canonical_name and trim(c.county) = trim(b.canonical_county) limit 1
    ) end as ccc_open_to_public,
    case when b.canonical_source = 'csp' then (
      select s.subtype from public.csp_parks s
      where s.unit_name = b.canonical_name limit 1
    ) end as csp_subtype,
...
select
  ...
  case
    -- Admin-pinned canonical wins over all spatial logic
    ...
    -- Fall back to spatial inference (unchanged)
  ...
  -- Tokens when a name-match actually determined the bucket
  case
    when can.source is not null and cardinality(can.shared_tokens) > 0 then can.shared_tokens
    when can.source is not null then null  -- explicit pin but no tokens overlap
  ...
  -- Pass-through metadata: canonical overrides spatial when set
  ...
  -- Admin-pinned flag for consumers that want to know
...
'Access-phase classification per us_beach_points row. Query-time, no persistence. Admin-pinned canonical_* columns win over spatial inference. Priority (name-match-first): canonical_pinned > cpad_named (100m + token) > ccc_named (200m + token) > cpad_plain (100m) > ccc_plain (200m) > csp (100m) > tribal (100m) > plz (bbox) > reclaim_cpad (300m + token) > reclaim_ccc (300m + token) > orphan. See project_pipeline_phases.md and migrations 20260425_beach_access_source_view.sql + 20260424_view_respects_canonical.sql.';
