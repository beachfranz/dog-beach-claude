-- Governance: cross-source override + promotion to locations_stage (2026-04-25)
--
-- Two changes wired into populate_from_park_url():
--
--   1. CROSS-SOURCE OVERRIDE for governance:
--      park_url_buffer_attribution evidence (page-confirmed buffer rescue)
--      OUTRANKS name/governing_body sources. Rationale: textually confirmed
--      that the source CPAD's webpage describes this specific beach is
--      stronger evidence than name-pattern heuristics or generic governing-
--      body inference. CPAD-direct (when it exists) and manual remain
--      higher precedence; we don't override those.
--
--      Per resolution-rules-design memory, governance precedence:
--        manual > cpad > park_url_buffer_attribution > governing_body > name
--      This migration encodes the third tier; the rest is existing behavior.
--
--   2. PROMOTER: after canonical resolution, write the canonical
--      governance evidence row's claimed_values back to
--      locations_stage.governing_body_name + governing_body_type so the
--      beach record itself reflects the resolved value.
--      Handles two key conventions:
--        - park_url evidence uses 'governing_body_name' / 'governing_body_type'
--        - older sources use 'name' / 'type' (per phase1_resolver.sql)
--      COALESCE between both.
...
begin
  ...
  -- Pass 2: set winners (for governance, override name/governing_body)
  -- Step 2a: clear name/governing_body governance canonicals where a
  -- park_url winner is about to claim. This is the cross-source override.
  update public.beach_enrichment_provenance victim
     set is_canonical = false
   where victim.field_group = 'governance'
     and victim.is_canonical = true
     and victim.source in ('name','governing_body')
     and exists (
       select 1
         from _resolver_ranked r
  ...
  -- Step 2b: now set the park_url winner. The OR side handles the case
  -- where we just cleared the prior name/governing_body canonical.
  update public.beach_enrichment_provenance e
  ...
  -- Two key conventions handled by COALESCE:
  --   • our park_url evidence uses 'governing_body_name' / 'governing_body_type'
  --   • older sources use 'name' / 'type' (per phase1_resolver.sql)
  update public.locations_stage s
   ...
   where e.fid = s.fid
     ...
     and (
       ...
       or s.governing_body_type is distinct from coalesce(
         e.claimed_values->>'governing_body_type',
         e.claimed_values->>'type'
       )
     );
...
comment on function public.populate_from_park_url(int) is
  'Layer 2 populator + Tier 1 resolver + governance promoter. Emits dogs/practical/governance evidence from park_url_extractions. Resolves canonical via Tier 1 rules (overlay-demote, containing-with-Beach, similarity, area). For governance, park_url_buffer_attribution overrides name/governing_body sources. Promotes canonical governance to locations_stage.governing_body_name/type. Fires multi_cpad_disagreement and source_governing_mismatch review flags.';

-- Backfill: re-run on all beaches
