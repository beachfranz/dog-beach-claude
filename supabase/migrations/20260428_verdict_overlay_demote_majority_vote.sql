-- Two cascade fixes:
--
-- (#7) compute_dogs_verdict: when a CCC point sits inside multiple CPAD
-- polygons, demote environmental overlays (Marine Parks, Ecological
-- Reserves, Wildlife Areas/Refuges, MPAs) below access-grade polygons.
-- Overlay still wins as a fallback if it's the only candidate.
--
-- (#6) all_coastal_features_lite: replace the OSM/UBP→CCC inheritance
-- aggregation `bool_or yes / bool_and no` (which let a single yes-CCC
-- flip a 7-CCC polygon) with majority-with-tie-null. Same change in
-- both UNION branches.

-- ── #7: compute_dogs_verdict CPAD lookup ────────────────────────────
...
begin
  ...
  select
   ...
   order by
     -- Non-overlay polygons first; overlay (Marine Park / Ecological
     -- Reserve / Wildlife Area / MPA) only wins if nothing else covers.
  ...
  -- ── Operator default policy (Pass 4) ─────────────────────────────
...
-- ── #6: all_coastal_features_lite — majority-with-tie-null ──────────
...
as $$
  -- OSM: associated-CCC majority, else operator default.
  select 'beach'::text,
  ...
  -- UBP-CA: associated-CCC majority, else operator default.
  select 'beach',
         'ubp/' || u.fid::text,
         u.name, 'beach', 'ubp',
         op.canonical_name,
