-- Bump operator_exception weight 0.68 → 1.0. Per-beach primary-source
-- quote naming this specific beach is the strongest non-manual signal
-- in the cascade. Note: not a hard override — three confident upstream
-- signals can still outvote it (e.g., Coronado City Beach where
-- cpad_web + ccc_llm + ccc_native all say yes from generic agency
-- pages). For full override semantics, use cpad_manual.
