-- Pass 9 — drop the buggy exception-contains-beach branch from the
-- exception-name matcher.
--
-- The exception matcher in compute_dogs_verdict_core had three OR
-- branches:
--   1. trigram similarity ≥ 0.65
--   2. clean(beach) LIKE '%clean(exc)%'  (beach name contains exception)
--   3. clean(exc) LIKE '%clean(beach)%'  (exception name contains beach)
--
-- Branch 3 is semantically backwards: when the exception name is
-- LONGER and contains the beach name, the exception is almost
-- certainly about a sub-area of the beach, not the beach itself.
--
-- The bug fires hardest on state beaches because clean_beach_name()
-- strips "State Beach" entirely. "Pismo State Beach" cleans to just
-- "pismo" (5 chars). Then any exception name containing "pismo" —
-- e.g., "Pismo Dune Preserve" (a sub-area inside Pismo State Beach
-- that prohibits dogs) — matches via branch 3 and overrides the
-- correct allowed default for the parent beach.
--
-- Truth-set comparison flagged 9 LIKELY_OUR_ERROR_no cases, mostly
-- CDPR state beaches. Pismo State Beach was the canonical example:
-- CDPR's exceptions[] explicitly says "Pismo State Beach: allowed"
-- but the cascade returned 'no' because "Pismo Dunes Natural Preserve"
-- (prohibited) matched first via branch 3.
--
-- Quantified before this migration:
--   Total exception matches across active surface: 580
--   Branch-3-only matches (no other branch fires): 65
--   Of those, branch-3-only with clean beach ≤5 chars: 58
--   ~ the buggy short-name pattern is dominant in branch-3-only.
--
-- Fix: drop branch 3 from both matchers (operator_exception and
-- cpad_unit_exception). Keep branches 1 (trigram) and 2 (beach
-- contains exception). Branch 1 catches exact matches (similarity=1)
-- and good fuzzy matches; branch 2 handles the legitimate "beach
-- name is more specific than exception name" case (e.g., "North
-- Salmon Creek Beach" matching exception "Salmon Creek").
...
begin
  ...
  if v_unit_id is not null then
    -- cpad_unit_exception: drop branch 3
  ...
  if p_operator_id is not null and p_name is not null then
    -- operator_exception: drop branch 3
    select case when e->>'rule' in ('off_leash','allowed','yes','restricted') then 'yes'
                when e->>'rule' in ('prohibited','no')                         then 'no' else null end
      into v_op_val
