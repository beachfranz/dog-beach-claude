-- Path 3b-3: drop the public.beaches LEFT JOIN from find_beaches RPC.
-- Slug now lives on beaches_gold.location_id directly.

begin;

drop function if exists public.find_beaches(
  p_date date, p_lat double precision, p_lng double precision,
  p_leash text, p_limit integer, p_scored_only boolean
);
