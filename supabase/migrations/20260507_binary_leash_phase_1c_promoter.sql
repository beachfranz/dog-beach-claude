-- 20260507_binary_leash_phase_1c_promoter.sql
--
-- Binary leash schema migration — Phase 1C (promoter).
--
-- Extends promote_canonical_dogs_to_beach_dog_policy to write the new
-- has_on_leash and has_off_leash columns alongside legacy leash_policy.
--
-- The booleans flow from beach_field_consensus (computed by Phase 1B's
-- claims_derived UNION) -> normalize via _norm_bool -> beach_dog_policy
-- has_on_leash / has_off_leash columns. Phase 2 will surface these on
-- detail.html / Scout chat / admin tooling. Phase 3 retires leash_policy.
...
begin
  with consensus_dogs as (
    select gold_fid,
      ...
      -- New: per-binary winners from Phase 1B
