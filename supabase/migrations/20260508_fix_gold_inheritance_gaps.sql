-- 20260508_fix_gold_inheritance_gaps.sql
--
-- Audit on 2026-05-08 found that promote_to_gold drops three columns
-- on the floor: county_fips (not in INSERT), park_name (set to null
-- explicitly even though arena has it), and address_city (handled via
-- a different path that doesn't reach gold for non-CA states).
--
-- Fixes:
--  1. promote_to_gold: include county_fips and park_name from arena.
--  2. arena.park_name: backfill OR/WA from pad_us_units spatial join.
--  3. beaches_gold.county_fips, park_name: backfill from arena for any
--     active row where they're null.

begin;

-- 1) Backfill arena.park_name for any state where it's null, using
-- pad_us_units (the populated source of unit names). Pick the smallest
-- containing protected-area polygon. Only fills nulls — doesn't
-- overwrite existing values.
update public.arena a
   set park_name = sub.unit_name
  from (
    select a.fid, p.unit_name,
           row_number() over (partition by a.fid order by st_area(p.geom::geography)) as rn
      from public.arena a
      join public.pad_us_units p
        on st_contains(p.geom, st_setsrid(st_makepoint(a.lon, a.lat), 4326))
     where a.is_active and a.park_name is null
  ) sub
 where a.fid = sub.fid and sub.rn = 1 and a.park_name is null;

-- 2) Backfill beaches_gold from arena for the three columns
update public.beaches_gold g
   set park_name   = coalesce(g.park_name, a.park_name),
       county_fips = coalesce(g.county_fips, a.county_fips),
       name_source = coalesce(g.name_source, a.name_source)
  from public.arena a
 where a.fid = g.fid
   and g.is_active
   and (g.park_name is null  and a.park_name is not null
     or g.county_fips is null and a.county_fips is not null
     or g.name_source is null and a.name_source is not null);

-- 3) Update promote_to_gold to include county_fips and park_name in the
-- INSERT so future promotions don't repeat the gap. Also keeps
-- _emit_evidence_from_osm_amenities wired in (added 20260508).
      ...
      left join lateral (
        select nav_lat, nav_lon
         ...
         where group_id = coalesce(t.group_id, t.fid)
           and source_code = 'osm'
           and is_active = true
           and nav_lat is not null
           and nav_lon is not null
         limit 1
      ) osm on true
  ),
  ins as (
    insert into public.beaches_gold
      (fid, location_id, name, lat, lon, county_name, county_fips, source_code, source_id, group_id,
