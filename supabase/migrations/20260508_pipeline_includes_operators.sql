-- 20260508_pipeline_includes_operators.sql
-- Add populate_operators_for_state(state) as the FIRST step in
-- run_pipeline_for_state, after the precheck. Builds the operators
-- table from TIGER cities + counties before any populator that depends
-- on it runs.
...
begin
  ...
  -- NEW: build operators (cities + counties from TIGER) before any
  -- populator that joins against operators.
  ...
  return query select
    ...
    v_cities, v_counties, v_addr_n, v_nsrc_n,
...
comment on function public.run_pipeline_for_state(text, bigint[], boolean) is
  'Integrated state-launch pipeline: precheck (PAD-US, osm_landing, osm_amenities, tiger_places) → populate_operators_for_state (cities+counties from TIGER) → cluster → extras → promote_to_gold (with all populators, resolvers, OSM amenity emitter) → address_city PIP → name_source backfill → late-stage dedup → geom queue drain. LLM dog-policy enrichment for new operators is a separate step (extract_operator_dogs_policy.py / v2-enrich-operational); call after pipeline completes.';
