-- 20260508_safe_updates_compat.sql
--
-- Some Supabase session contexts (notably edge functions calling RPCs via
-- supabase-js) run with sql_safe_updates enabled, which rejects UPDATE
-- without a WHERE clause. The state-launcher edge function hit this on
-- populate_arena_group_id's "reset to singletons" line.
--
-- This migration rewrites the function with the SAME body but adds a
-- `where true` to the bare UPDATE. Sourced from pg_get_functiondef of
-- the live function on 2026-05-08; only that one line changed.
...
begin
  ...
  -- 1. Reset to singleton (group_id = fid). `where true` for safe-updates compat.
