-- 20260508_split_init_from_publish.sql
--
-- Split promote_to_gold into init + publish phases. State launches
-- need a HARD STOP between BEP enrichment and consumer-surface
-- publishing so a human can review extraction output before live data
-- ships.
--
-- New shape:
--   promote_to_gold(fids[], p_score, p_publish=true)
--     same signature as before but adds p_publish flag.
--     When p_publish=false: stops after populators+resolvers; no
--       compute_beach_field_consensus, no promote_canonical.
--     Default true preserves existing call-site behavior.
--
--   publish_canonical_for_fids(fids[]) returns counts
--     runs consensus + promote_canonical for the given fids.
--     Idempotent. Call this after extraction queue drains and human
--     review gate clears.
...
begin
  ...
  -- BEP enrichment loop (populators + resolvers always run)
  ...
  -- ★ PUBLISH PHASE: only when p_publish=true. Otherwise stops here
  -- (a human-review gate sits between BEP and consumer surface).
  if p_publish then
    foreach fid_iter in array p_fids loop
...
-- Standalone publisher: explicit, on-demand, post-extraction
create or replace function public.publish_canonical_for_fids(p_fids bigint[])
returns table(fids_published bigint, beach_dog_policy_changed bigint, beach_amenities_changed bigint)
language plpgsql
security definer
as $function$
declare
...
begin
  ...
  select count(*) into v_bdp from public.beach_dog_policy
   where arena_group_id = any(p_fids) and source = 'auto_promoted_from_consensus';
  select count(*) into v_ba from public.beach_amenities
   where arena_group_id = any(p_fids) and source = 'auto_promoted_from_consensus';

  return query select v_pub, v_bdp, v_ba;
end;
$function$;

grant execute on function public.publish_canonical_for_fids(bigint[]) to anon, authenticated, service_role;
