-- 20260508_wire_operators_populator.sql
-- Adds populate_from_operators_gold(fid) to the populator chains in
-- promote_to_gold and refire_bep_cascade so the generic operator-via-
-- PAD-US/county/jurisdictions path runs automatically for any state.
...
begin
  ...
  foreach fid_iter in array p_fids loop
    ...
    perform public.populate_from_operators_gold(fid_iter);  -- ★ NEW: state-agnostic operator → BEP via PAD-US/county/jurisdictions
...
begin
  ...
  foreach fid_iter in array p_fids loop
    ...
    perform public.populate_from_pad_us_gold(fid_iter);
    perform public.populate_from_park_operators_gold(fid_iter);
    perform public.populate_from_operators_gold(fid_iter);  -- ★ NEW
    perform public.populate_from_research_gold(fid_iter);
    perform public.populate_from_park_url_gold(fid_iter);
    perform public.populate_from_park_url_governance_gold(fid_iter);
    perform public.populate_from_unified_v1_gold(fid_iter);
