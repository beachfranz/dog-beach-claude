-- 20260508_wire_osm_amenities_emitter.sql
--
-- Adds public._emit_evidence_from_osm_amenities(fid) to the populator
-- chain in promote_to_gold and refire_bep_cascade. Without this, OSM
-- amenity evidence never lands in BEP and the practical resolver has
-- nothing to consume — beach_amenities stays empty for any state where
-- the amenity emitter wasn't fired manually.
--
-- This is the explicit wiring fix for the OR/WA amenity gap surfaced
-- during the 2026-05-08 enrichment audit.
...
begin
  ...
  foreach fid_iter in array p_fids loop
    ...
    perform public._emit_evidence_from_osm_amenities(fid_iter);  -- ★ NEW: structural amenity signal
...
begin
  ...
  foreach fid_iter in array p_fids loop
    ...
    perform public.populate_from_city_dog_policy_gold(fid_iter);
    perform public.populate_from_county_dog_policy_gold(fid_iter);
    perform public._emit_evidence_from_osm_amenities(fid_iter);  -- ★ NEW
