-- 20260508_wire_pad_us_into_pipeline.sql
--
-- Wire populate_from_pad_us_gold into promote_to_gold and refire_bep_cascade
-- so PAD-US BEP rows get emitted alongside CPAD on every promotion + cascade.
--
-- Per state-agnostic principle: PAD-US runs for all states; CPAD only
-- produces rows for CA (cpad_units only contains CA data — no behavior
-- change for non-CA fids).

begin;

-- promote_to_gold: add populate_from_pad_us_gold to the populator loop
...
begin
  ...
  foreach fid_iter in array p_fids loop
    ...
    perform public.populate_from_pad_us_gold(fid_iter);  -- ★ NEW: PAD-US parallel signal
...
-- refire_bep_cascade: add populate_from_pad_us_gold to the populator suite
...
begin
  ...
  foreach fid_iter in array p_fids loop
    ...
    perform public.populate_polygon_containment_gold(fid_iter);
    perform public.populate_from_cpad_gold(fid_iter);
    perform public.populate_from_pad_us_gold(fid_iter);  -- ★ NEW
    perform public.populate_from_park_operators_gold(fid_iter);
    perform public.populate_from_research_gold(fid_iter);
    perform public.populate_from_park_url_gold(fid_iter);
    perform public.populate_from_park_url_governance_gold(fid_iter);
