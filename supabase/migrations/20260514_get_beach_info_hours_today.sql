-- 20260514_get_beach_info_hours_today.sql
--
-- Add `hours_today` (array of 24 hourly rows for current_date) to
-- get_beach_info so beach.html's Best Window card can render the
-- score bar chart inline. Just the fields the bar renderer needs:
-- local_hour, hour_score, hour_status, is_in_best_window.
-- Returns empty array when the beach isn't scoreable.
