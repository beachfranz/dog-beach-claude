-- 20260514_get_beach_info_hours_today_metrics.sql
--
-- Extend `hours_today` payload to include per-metric status + value
-- fields (tide / wind / rain / crowd / temp / UV / sand / asphalt).
-- beach.html's new Advisories card needs these to compute time-block
-- groupings per metric — the same logic detail.html uses for its
-- status board.
--
-- The hours_today array shape grows from {local_hour, hour_score,
-- hour_status, is_in_best_window, is_now} to also include each
-- metric's status + raw value.
