-- 20260514_get_beach_info_with_alternatives.sql
--
-- Add `alternatives` array to get_beach_info response so beach.html
-- can render the "try these instead" card. Mirrors what detail.html
-- gets via get-beach-detail edge function. Frontend uses fid, name,
-- distance_km, has_on_leash, has_off_leash for triggers; day_status
-- + best_window_label included for parity but mostly null for the
-- non-scoring beach.html surface.
