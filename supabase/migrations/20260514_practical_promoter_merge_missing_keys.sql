-- 20260514_practical_promoter_merge_missing_keys.sql
--
-- When the canonical practical BEP row lacks specific keys (e.g.
-- parking_fee, parking_subtype) but lower-confidence rows have them,
-- fill the missing keys from the best non-canonical row that carries
-- each one. Preserves "best source per claim" without changing the
-- canonical-row tier ordering.
--
-- Observed case (2026-05-14): Silver Strand has an old_school_llm
-- canonical row at conf=0.80 carrying parking_type='mixed' but no
-- parking_fee or parking_subtype. The osm_amenities_v1 emitter
-- (conf=0.65) carries OSM fee+subtype tags from the closest parking
-- POI. Without this merge, those fields stay null on beach_amenities
-- even though the data is in BEP. Same applies to has_lifeguards /
-- has_disabled_access / other fields when an older source ranks
-- higher than the newer enrichment.
...
begin
  ...
  -- Canonical row's claimed_values (the winner of _resolve_practical_gold).
  canon_payload as (
  ...
  -- Supplemental: build a per-fid jsonb of keys present in non-canonical
  -- rows but NOT in canonical. Each key takes the value from the highest-
  -- confidence non-canonical row that has it.
  supplemental as (
  ...
  -- Merge: supplemental first, canonical wins on conflict.
  merged as (
    select c.fid,
