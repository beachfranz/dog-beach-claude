-- 20260523_pip_membership_tcs_town.sql
--
-- Extends refresh_beach_polygon_membership to spatial-join beaches
-- against public.tiger_county_subdivisions (the MCD layer), recording
-- memberships with polygon_kind='tcs_town'. Per directive 2026-05-23
-- LATE: "be sure to model mcds using same process as we use for C1s".
--
-- The cousub PIP block uses the geom_geog GiST index directly (vs
-- existing TIGER PLACE block which casts j.geom::geography inline)
-- because tiger_county_subdivisions is loaded with geom_geog always
-- populated (gap #43 lesson — guard against silent NULL geog).
--
-- New block is always-on (no town-strong gate). In county-strong
-- states, tiger_county_subdivisions is empty for that state → the join
-- produces zero rows → no harm. The town-strong filter is enforced at
-- the operator-promoter level (Task 5).
...
declare
  ...
  v_total_tcs     bigint := 0;   -- NEW: TIGER county subdivisions (MCD towns)
...
begin
  ...
  -- ── TIGER county subdivisions / MCDs (state-filtered, geom_geog) ───
  -- NEW 2026-05-23 LATE (gap #46). Town-strong states (NH/CT/MA/ME/RI/
  -- VT/NJ/PA) hold dog-policy authority at the town tier, which TIGER
  -- carries in the COUSUB layer (not PLACE). In county-strong states
  -- this table is empty → join produces zero rows.
  with beaches as (
    select fid, geom, geom::geography as geog
      from public.beaches_gold
     where state = p_state and is_active = true
