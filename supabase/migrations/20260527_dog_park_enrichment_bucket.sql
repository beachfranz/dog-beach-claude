-- 20260527_dog_park_enrichment_bucket.sql
--
-- Add "enrichment" bucket — separate from basic features — to score
-- "built for dogs" amenities (Franz 2026-05-27 LATE, option C).
--
-- New columns:
--   has_agility       — agility equipment (jumps/tunnels/weaves/A-frame)
--   has_water_play    — splash pad / dog fountain / swimmable pond
-- Both NULL = unknown (no scoring impact until curated).
--
-- New positive bucket: enrichment (max 9)
--   agility:     +3 flat when has_agility=true
--   water_play:  sliding 0-6 when has_water_play=true, same shade_need
--                formula (max of UV-band and feels-like-temp need)

alter table public.dog_park_dog_policy
  add column if not exists has_agility boolean,
  add column if not exists has_water_play boolean,
  add column if not exists has_picnic_tables boolean;

comment on column public.dog_park_dog_policy.has_agility is
  'True if park has agility equipment (jumps, tunnels, weave poles, A-frame, etc). '
  'Drives the flat +3 enrichment-agility positive in v2 scoring.';

comment on column public.dog_park_dog_policy.has_water_play is
  'True if park has cooling water play features (splash pad, dog fountain, '
  'swimmable pond, etc). Drives heat-sliding enrichment-water_play positive.';

comment on column public.dog_park_dog_policy.has_picnic_tables is
  'True if park has picnic tables (owner can stay/eat → dog gets longer visit). '
  'Drives the flat +2 comfort-picnic_tables positive in v2 scoring.';
...
begin
  ...
  -- Features bucket (max 22): fence 8 + water 4 + shade 6 + comfort 4
  ...
  -- Enrichment bucket (max 9): agility 3 + water_play 6
