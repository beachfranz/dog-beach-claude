-- 20260527_dog_park_scoring_v2_curves2.sql
--
-- Round-2 curve adjustments per Franz 2026-05-27 LATE:
--   1. Wind bell: 0mph → 0 (not 4). Peak 6 at 5mph. Rises sharply to 5,
--      falls more gradually to 15mph → 0.
--   2. Feels-like uses the same bell as temp (already does — peak 72°F).
--      Hot/cold negatives now branch off the same bell SHAPE rather than
--      hard-threshold linear segments.
--   3. Harsh wind starts at 20mph (not 10). Linear 0.5pts/mph,
--      maxed at 40mph (→ 10pts).
--   4. Cold starts at 50°F (not 40°F). Linear 50 → 25°F maps 0 → 10.
--   5. CAUTION tier now caps the score at 60 (just below "go" range).
--      Tier hierarchy: no_go → 30, caution → 60, advisory/go → no cap.
...
begin
  ...
  -- temp: bell — peak 10 at 72°F, falloff ±20°F (feels-like)
  ...
  -- wind: 0mph → 0, peak 6 at 5mph (steep rise 0→5), falloff 5→15mph → 0
  if v_now.wind_speed is not null then
    if v_now.wind_speed <= 5 then
      v_weather_pos := v_weather_pos + greatest(0, round(v_now.wind_speed * 6 / 5.0)::int);
    else
      v_weather_pos := v_weather_pos + greatest(0, round(6 - (v_now.wind_speed - 5) * 0.6)::int);
    end if;
  end if;
  -- WMO code lookup
  ...
  -- compound heat (feels-like above bell range): linear 85 → 100°F → 0 → 5
  ...
  -- wind harsh: 20mph → 0, linear 0.5pts/mph, 40mph → 10
  if v_now.wind_speed is not null and v_now.wind_speed > 20 then
    v_harsh_neg := v_harsh_neg
      + least(10, greatest(0, round((v_now.wind_speed - 20) * 0.5)::int));
  end if;
  -- precip chance: linear 30% → 100% → 0 → 10
  ...
  -- cold: starts at 50°F, 25°F → 10 (linear 50 → 25)
  if v_now.feels_like is not null and v_now.feels_like < 50 then
    v_harsh_neg := v_harsh_neg
      + least(10, greatest(0, round((50 - v_now.feels_like) * 0.4)::int));
  ...
  -- ── Status tier caps (Franz 2026-05-27 LATE: cautions matter) ─────
  -- Tier hierarchy: no_go → 30, caution → 60, advisory/go → no cap.
  if v_now.hour_status = 'no_go' then
    v_gate_fired := true;
    v_cap := 30;
  elsif v_now.hour_status = 'caution' then
    v_cap := 60;
  else
    v_cap := 100;
  end if;
  if v_gate_fired then v_cap := least(v_cap, 30); end if;
  ...
  return public._score_v2_compose(
    ...
    p_status_gate_fired := v_gate_fired or v_cap < 100,
...
-- ════════════════════════════════════════════════════════════════════
-- Hourly scorer — same curves, applied per-hour with caution-tier cap
-- ════════════════════════════════════════════════════════════════════
...
begin
  ...
  with per_hour as (
    select
      ...
      least(22,
        -- temp bell, peak 10 at 72°F
        greatest(0, round(10 * (1 - least(abs(coalesce(h.feels_like, 72) - 72), 20) / 20.0))::int)
        -- wind: 0mph→0, peak 6 at 5mph, falloff to 15mph→0
        + case when h.wind_speed is null then 0
               when h.wind_speed <= 5 then greatest(0, round(h.wind_speed * 6 / 5.0)::int)
               else greatest(0, round(6 - (h.wind_speed - 5) * 0.6)::int)
          end
        -- WMO lookup
        + case h.weather_code
            when 0 then 6 when 1 then 5 when 2 then 4 when 3 then 2
      ...
      least(15,
        case when h.wind_speed > 20
             then least(10, greatest(0, round((h.wind_speed - 20) * 0.5)::int))
             else 0 end
      ...
      -- Status-tier cap: no_go→30, caution→60, else 100
      case when h.hour_status = 'no_go' then 30
           when h.hour_status = 'caution' then 60
  ...
  scored as (
    select *,
      greatest(0, least(
        case when gate_fired then 30 else status_cap end,
        50 + features_pos + weather_pos + window_pos
