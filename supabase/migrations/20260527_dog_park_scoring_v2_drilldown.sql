-- 20260527_dog_park_scoring_v2_drilldown.sql
--
-- Break the two lump positives into named sub-components for the
-- tug-of-war chart (Franz 2026-05-27 LATE):
--
--   pos_weather (max 22)  →  pos_temp (10) + pos_wind (6) + pos_sky (6)
--   pos_features (max 20) →  pos_fence (9) + pos_water (5) + pos_comfort (6)
--
-- Amenity grouping:
--   fence    = perimeter fence (6) + double gate (3) — safety
--   water    = drinking water (5)
--   comfort  = grass surface (3) + dog-size areas (2) + large enough (1)
--
-- Total positive cap is unchanged (22 weather + 20 features + 5 window + 3 drive = 50).
-- Both compute_dog_park_score_v2 (point) and compute_dog_park_hourly_v2 (hourly)
-- emit the new sub-fields. Legacy pos_weather + pos_features sums are
-- retained for backwards compat.

begin;

-- ════════════════════════════════════════════════════════════════════
-- Point scorer (used by find.html via nearest_dog_parks RPC)
-- ════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── Weather positive — three named sub-components ────────────────
  ...
  -- ── Features positive — three named sub-components ───────────────
  ...
  if v_now.asphalt_temp is not null then
    v_heat_neg := v_heat_neg + least(15, greatest(0, round((v_now.asphalt_temp - 100) / 2.0)::int));
    if v_now.asphalt_temp >= 125 then v_gate_fired := true; end if;
  end if;
  if v_now.uv_index is not null then
    v_heat_neg := v_heat_neg + least(10, greatest(0, round((v_now.uv_index - 5) * 10 / 6.0)::int));
    if v_now.uv_index >= 11 then v_gate_fired := true; end if;
  end if;
  if v_now.feels_like is not null and v_now.feels_like > 85 then
    v_heat_neg := v_heat_neg + least(5, greatest(0, round((v_now.feels_like - 85) / 3.0)::int));
  end if;
  ...
  if v_now.hour_status = 'no_go' then v_gate_fired := true; v_cap := 30;
  elsif v_now.hour_status = 'caution' then v_cap := 60;
  else v_cap := 100; end if;
  ...
  return public._score_v2_compose(
    ...
    -- Drilldown sub-components (Franz 2026-05-27 LATE)
...
-- ════════════════════════════════════════════════════════════════════
-- Hourly scorer (used by dog-park.html for the tug-of-war chart)
-- ════════════════════════════════════════════════════════════════════
...
declare
  ...
  v_view_date date := coalesce(p_date, current_date);
  -- Static (constant across hours) features sub-components
  v_fence_pos integer := 0; v_water_pos integer := 0; v_comfort_pos integer := 0;
  v_features_pos integer := 0;
...
begin
  ...
  with per_hour as (
    select h.local_hour, h.hour_label, h.hour_status,
      ...
      (h.hour_status = 'no_go' or coalesce(h.asphalt_temp, 0) >= 125
  ...
  select jsonb_agg(jsonb_build_object(
    ...
    'drive', v_drive,
    -- Drilldown sub-components (NEW)
    'pos_temp', temp_pos, 'pos_wind', wind_pos, 'pos_sky', sky_pos,
    'pos_fence', v_fence_pos, 'pos_water', v_water_pos, 'pos_comfort', v_comfort_pos,
    -- Raw weather snapshot for tooltip
  ...
  return jsonb_build_object(
    ...
    -- Static feature sub-totals (constant across all hours)
    'pos_fence', v_fence_pos, 'pos_water', v_water_pos, 'pos_comfort', v_comfort_pos,
