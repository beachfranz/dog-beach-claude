-- 20260527_dog_park_scoring_v2_heat_recal.sql
--
-- Recalibrate heat_uv negative thresholds to match real-world dog-safety
-- references (Franz 2026-05-27 LATE):
--
--   - UV: keep current curve (5 → 0, 11 → 10) — already aligned with
--     the EPA / Coppertone UV Index categories (Moderate cap → Extreme).
--
--   - Asphalt: raise penalty start from 100°F to 115°F. The 100°F floor
--     was triggering on benign 78°F afternoons; per the Weave pavement
--     chart, paw-burn risk really starts at 125°F asphalt (= 77°F air
--     with sun). Cap at 135°F (the "skin destruction in 60s" point).
--     Gate at 125°F is unchanged.
--       115°F → 0  |  120°F → 4  |  125°F → 7.5  |  135°F → 15
--
--   - Hot feels-like: raise penalty start from 85°F to 90°F. Active-play
--     heat-stroke risk for typical dogs starts around 90°F feels-like.
--       90°F → 0  |  95°F → 2.5  |  100°F → 5
--
-- Net effect: a 78°F afternoon with UV 7 + 110°F asphalt drops from
-- heat_uv ≈ 8 down to ≈ 3 (just the UV part — which is correct, that's
-- "High" tier sun).
...
begin
  ...
  -- ── Heat/UV negative (recalibrated to dog-safety thresholds) ──────
  -- Asphalt 115°F → 0 (was 100°F); 135°F → 15. Per Weave pavement chart.
  if v_now.asphalt_temp is not null and v_now.asphalt_temp > 115 then
    v_heat_neg := v_heat_neg
      + least(15, greatest(0, round((v_now.asphalt_temp - 115) * 0.75)::int));
    if v_now.asphalt_temp >= 125 then v_gate_fired := true; end if;
  end if;
  -- UV: unchanged (5 → 0, 11 → 10) — already EPA-aligned.
  ...
  -- Hot feels-like: 90°F → 0 (was 85°F); 100°F → 5.
...
begin
  ...
  with per_hour as (
    select h.local_hour, h.hour_label, h.hour_status,
      ...
      -- Heat/UV negative (recalibrated): asphalt 115°→0, 135°→15; UV unchanged; feels 90→0,100→5
      least(25,
        case when h.asphalt_temp > 115
             then least(15, greatest(0, round((h.asphalt_temp - 115) * 0.75)::int))
             else 0 end
