-- 20260527_dog_park_scoring_v2_shade.sql
--
-- Major rewrite of features bucket + gotchas bucket per Franz 2026-05-27 LATE:
--
-- 1. Kill double-counting on fence + water — drop the negative-side
--    gotchas. Having a fence/water is rewarded only on the positive
--    side; not having one is just the absence of that reward.
--
-- 2. Add shade as first-class feature, UV-scaled (it's only valuable
--    when the sun is strong):
--      UV <4  → 0  · UV 4-6 → 2  · UV 7-9 → 3  · UV 10+ → 4
--
-- 3. Add no-shade-in-sun gotcha, also UV-scaled (mirror of shade):
--      UV <6  → 0  · UV 6-7 → 2  · UV 8-9 → 3  · UV 10+ → 4
--
-- 4. Drop the no-shade-on-pavement gotcha — redundant with no-shade-in-sun.
--
-- features bucket (max 20):
--   fence    8   = perimeter (6) + double_gate (2)
--   water    4   = drinking water
--   shade    4   = has_shade + UV >= 4 (sliding scale)
--   comfort  4   = grass (2) + dog-size area (1) + spacious ≥1ac (1)
--
-- gotchas bucket (max 4):
--   no shade in sun:  4 max  (has_shade=false AND UV ≥ 6)
...
begin
  ...
  v_shade := v_policy.has_shade;  -- NULL = unknown
  ...
  -- Features bucket (max 20): fence 8 + water 4 + shade 4 + comfort 4
  ...
  -- Shade: UV-scaled positive (only valuable when sun is strong)
  if v_shade = true and v_now.uv_index is not null then
    v_shade_pos := case
      when v_now.uv_index < 4 then 0
      when v_now.uv_index < 7 then 2
      when v_now.uv_index < 10 then 3
      else 4
    end;
  end if;
  if v_surface = 'grass'        then v_comfort_pos := v_comfort_pos + 2; end if;
  ...
  -- Wind harsh (unchanged: 19-24=4, 25-34=6, 35+=10)
  if v_now.wind_speed is not null then
  ...
  -- Gotchas (NEW): no-shade-in-sun, UV-scaled (mirror of shade positive)
  if v_shade = false and v_now.uv_index is not null then
    v_gotchas_neg := v_gotchas_neg + case
  ...
  v_gotchas_neg := least(v_gotchas_neg, 4);
  ...
  v_neg := jsonb_build_object(
    ...
    'gotchas', jsonb_build_object('value', v_gotchas_neg, 'max', 4));
...
declare
  ...
  v_view_date date := coalesce(p_date, current_date);
  -- Static sub-bucket totals (constant across hours)
  v_fence_pos integer := 0; v_water_pos integer := 0;
  v_comfort_pos integer := 0;
  -- Shade is NOT static — varies by hour's UV. Computed per-hour below.
...
begin
  ...
  with per_hour as (
    select h.local_hour, h.hour_label, h.hour_status,
      ...
      -- Shade positive: UV-scaled (only valuable when sun matters)
      case when v_shade = true and h.uv_index is not null then
        case
          when h.uv_index < 4 then 0
          when h.uv_index < 7 then 2
          when h.uv_index < 10 then 3
          else 4
        end
      else 0 end as shade_pos,
      least(25,
      ...
      -- Gotchas: no-shade-in-sun (mirror of shade positive, UV-scaled)
      case when v_shade = false and h.uv_index is not null then
        case
  ...
  composed as (
    select *,
      ...
      v_fence_pos + v_water_pos + shade_pos + v_comfort_pos as features_total
    from per_hour
  ),
  scored as (
    select *, greatest(0, least(status_cap,
        50 + features_total + weather_pos + window_pos
           - heat_uv_neg - harsh_neg - gotchas_neg + v_drive)) as score from composed
  )
  select jsonb_agg(jsonb_build_object(
    ...
    'pos_weather', weather_pos, 'pos_features', features_total, 'pos_window', window_pos,
