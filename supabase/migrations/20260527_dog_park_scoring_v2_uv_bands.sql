-- 20260527_dog_park_scoring_v2_uv_bands.sql
--
-- Scale UV direct-penalty back to a banded curve matching the dog-park
-- threat profile (Franz 2026-05-27 LATE):
--
--   UV 0-5  Low/Moderate    → 0
--   UV 6-7  High            → 1
--   UV 8-9  Very High lo    → 3
--   UV 10   Very High top   → 4
--   UV 11+  Extreme         → 10 (+ no_go gate fires)
--
-- UV's also still driving asphalt heat indirectly (the asphalt model
-- uses UV as the solar-load proxy) and still triggers the asphalt
-- gotcha at UV ≥ 6 on hard-surfaced parks. Those linkages unchanged.
...
begin
  ...
  -- UV: banded curve — 0/0/1/3/4/10 by EPA tier (Franz 2026-05-27 LATE)
...
begin
  ...
  with per_hour as (
    select h.local_hour, h.hour_label, h.hour_status,
      ...
      -- Heat/UV negative — UV uses banded curve (Franz 2026-05-27 LATE)
      least(25,
