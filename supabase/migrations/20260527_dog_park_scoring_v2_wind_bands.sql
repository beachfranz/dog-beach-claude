...
--   19-24    Fresh Breeze            →  +0 / -5  (discourages outdoor activities)
--   25-34    Strong / hazardous      →  +0 / -8
--   35+      Gales                   →  +0 / -10
--
...
begin
  ...
  if v_now.wind_speed is not null then
    v_harsh_neg := v_harsh_neg + case
      ...
      when v_now.wind_speed < 25 then 5
      when v_now.wind_speed < 35 then 8
      else 10
    end;
  end if;
    ...
    select h.local_hour, h.hour_label, h.hour_status,
      ...
      least(15,
        ...
        case
          ...
          when h.wind_speed < 25 then 5
          when h.wind_speed < 35 then 8
          else 10
        end
