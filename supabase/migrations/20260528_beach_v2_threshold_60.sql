-- 20260528_beach_v2_threshold_60.sql
--
-- Two coupled changes to beach v2 best-window (Franz 2026-05-28):
--
-- 1. Lower min_score threshold for the beach best-window finder from
--    70 → 60. Most beaches' v2 scores cluster in the 60s (Stoneboro
--    Beach example: 14 hours of 57-69, none reach 70). Threshold 60
--    means "above caution" — a meaningful "best" pick — and grows
--    v2 best-window coverage from 448 → 529 of 849 CA active beaches.
--
-- 2. Stop falling back to v1's best_window_label when v2 has no
--    qualifying window. Borrowing v1's looser pick was misleading —
--    surfaced labels that didn't match v2's actual scoring. Now: if
--    v2 has nothing, write NULL → page says "no clear best window
--    today" honestly.

begin;

-- Update beach compute to use threshold 60
...
begin
  ...
  -- Threshold lowered 70 → 60 for beaches (Franz 2026-05-28). Most CA beach
  -- v2 scores cluster in the 60s; >=70 was too strict.
  v_bw := public._find_best_window_v2(coalesce(v_hours, '[]'::jsonb), 60, 2, 5);
...
-- Apply function: ALWAYS write, even when v2 has nothing.
-- Stops the v1 fallback that was preserving misleading wide labels.
...
begin
  ...
  -- ALWAYS write — null v_label means honest "no clear best window today".
  -- Removes the v1 fallback that was preserving misleading wide labels.
