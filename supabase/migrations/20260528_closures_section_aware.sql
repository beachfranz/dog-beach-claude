-- 20260528_closures_section_aware.sql
--
-- Add section-awareness to closures (Franz 2026-05-28 level #2):
--   - _beach_closures_from_policy preserves which section the closure
--     targets (sand / water / etc.) in a `section` field.
--   - New helper _v2_first_closure_section(closures, date, hour) returns
--     the section of the first matching closure (or null).
--   - compute_beach_hourly_v2 adds closed_section to each per-hour output
--     so the consumer can label it accurately.
--
-- Practical effect on the UI: "Sand off-limits right now — reopens 6pm"
-- instead of the conflated "Closed right now — reopens 6pm".

begin;

-- ── First-matching-closure section lookup ───────────────────────────

create or replace function public._v2_first_closure_section(
  p_closures jsonb,
  p_date     date,
  p_hour     int
...
-- ── _beach_closures_from_policy: tag closures with section='sand' ──
...
begin
  if p_dogs_allowed = 'no' then
    return '[{"kind":"daily_all","section":"global","reason":"dogs_prohibited"}]'::jsonb;
  end if;
...
-- ── compute_beach_hourly_v2: emit closed_section per hour ──────────
-- Surgical change: add `closed_section` field to each hour's jsonb output.
