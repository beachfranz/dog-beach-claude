-- 2026-06-06 — Drop the old-logic 30/60 hard caps from v2 scoring.
--
-- The asphalt/UV/heat gates were a carryover from the v1 absolute-100 model.
-- The v2 tug-of-war already encodes those signals via neg_heat_uv / neg_harsh.
-- A hard ceiling on top of that double-counts AND short-circuits the
-- composition the moment one metric hits an extreme.
--
-- After this migration:
--   * Per-hour score = pure 50 + positives - negatives + drive, clamped 0..100
--   * Per-hour status from score band: >=70 clear / >=50 advisory / >=30 caution / <30 no_go
--   * Day status = worst per-hour status across daylight hours
--   * gate_fired / status_cap fields drop out of the hourly JSON
--   * _score_v2_compose loses p_status_gate_fired / p_status_gate_cap params
...
-- 2. _score_v2_compose — drop cap params, clamp 0..100.
--    Signature changes, so DROP then CREATE.
DROP FUNCTION IF EXISTS public._score_v2_compose(integer, jsonb, jsonb, integer, boolean, integer);

CREATE OR REPLACE FUNCTION public._score_v2_compose(
  p_baseline integer DEFAULT 50,
  p_positives jsonb DEFAULT '{}'::jsonb,
  p_negatives jsonb DEFAULT '{}'::jsonb,
  ...
  WITH
    ...
    scored AS (
      SELECT p_baseline + pos.total - neg.total + p_drive_factor AS s_raw,
             pos.total AS pos_total, neg.total AS neg_total
        FROM pos, neg
    )
  SELECT jsonb_build_object(
    'score',           GREATEST(0, LEAST(100, s_raw)),
    'score_uncapped',  s_raw,
    'baseline',        p_baseline,
...
-- 3. compute_dog_park_hourly_v2 — strip gate_fired + status_cap.
...
begin
  ...
  with_status as (
    select *,
      case when is_closed then 'closed'
           else public._v2_status_from_score(score)
      end as status_v2
    from scored
  )
  ...
  ) order by local_hour) into v_hours from with_status;
...
-- 4. compute_beach_hourly_v2 — strip gate_fired + status_cap.
...
-- 5. compute_dog_park_score_v2 — drop v_gate_fired/v_cap; status from band.
...
begin
  ...
  v_now_status_v2 := case
    ...
    else public._v2_status_from_score((v_composed->>'score')::int)
  end;
...
-- 6. _find_best_window_v2 — drop gate_fired filter.
...
-- 7. apply_v2_best_window_to_recommendations — day_status_v2 from worst per-hour score band.
...
begin
  ...
  -- Day status = worst per-hour status across daylight hours.
  -- Per-hour status now derived from the score band (>=70 clear, >=50 advisory,
  -- >=30 caution, <30 no_go) via _v2_status_from_score.
...
-- 8. apply_v2_best_window_to_beach_recommendations — same swap.
...
begin
  ...
  -- Day status = worst per-hour status across daylight hours, derived from
  -- the score band (no per-metric aggregator, no v2_compute_day_status path).
  select case max(case public._v2_status_from_score(h.hour_score_v2)
      when 'no_go'    then 3
      when 'caution'  then 2
      when 'advisory' then 1
