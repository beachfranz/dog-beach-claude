-- 20260607c_find_beaches_dog_hours.sql
--
-- Extend find_beaches to return dogs_prohibited_end + timezone, so
-- find.html cards can render the dog-hours chip ("🚫 Dogs out 10am–
-- 4:30pm") with the prohibition window framed in the beach's own
-- local time.
--
-- Context: 2026-06-07 evening — many CA cards show "Best" grey at noon
-- because tier-3 limited-access beaches have current_hour_score=null
-- (dogs prohibited mid-day). The grey number explains "not current,"
-- but doesn't tell the user WHY. The chip closes the gap so users see
-- "Newport Municipal 92 Best · 🚫 Dogs out 10am–4:30pm" together —
-- both signals point at the same fact.
--
-- Sample (Newport / N Star / Bayshores Play / Santa Ana River Jetties
-- all share the OC LIMITED-ACCESS pattern):
--   dogs_allowed='mixed', dogs_prohibited_start='10:00:00',
--   dogs_prohibited_end='16:30:00'
--
-- Return-table signature changes → DROP + CREATE.
