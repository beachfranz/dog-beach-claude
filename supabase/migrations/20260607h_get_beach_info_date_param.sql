-- 20260607h_get_beach_info_date_param.sql
--
-- get_beach_info gains an optional p_date so beach.html can render for
-- a future-day-pill click coming from find.html. Default current_date
-- so existing callers (no second arg) are unaffected.
--
-- The JSON keys 'today' and 'hours_today' keep their names — renaming
-- them would ripple across 10+ render sites in beach.html. Semantically
-- they now mean "the selected day"; beach.html will branch on
-- isToday=(p_date == current_date) for the date-specific display
-- adjustments (fit badge label, chart 🐶 mark, Scout framing).
--
-- Per Franz 2026-06-07: clicking a Monday-pill card on find.html
-- should land on beach.html showing Monday's data, not today's.

-- Drop the prior single-arg overload so the new signature is unique
-- (PostgREST/PG can't disambiguate get_beach_info(8347) when both
-- (bigint) and (bigint, date DEFAULT NULL) exist).
DROP FUNCTION IF EXISTS public.get_beach_info(bigint);

CREATE OR REPLACE FUNCTION public.get_beach_info(
  p_fid  bigint,
...
AS $function$
  -- target_date defaults to TODAY at the BEACH's own timezone, not the
  -- DB server's UTC date. Without this, a viewer hitting beach.html
  -- at 7pm Pacific (= 02:00 UTC the next day) was getting tomorrow's
  -- data because current_date is UTC. Pinned: [[arena-group-id-is-fid-not-group]]
  -- is unrelated but reminds us that fid is the stable key here.
  WITH d AS (
    SELECT COALESCE(
      p_date,
  ...
  SELECT jsonb_build_object(
    ...
    'requested_date', (SELECT target_date FROM d),     -- echoed back so client can reason about it
...
GRANT EXECUTE ON FUNCTION public.get_beach_info(bigint, date) TO anon, authenticated;
