-- 20260609c — compute_beach_hourly_v2 emits V3 score alongside V2
--
-- WHY: Score-reform Phase 1 in the LIVE read path. The bulk writer
-- (apply_v2_best_window_to_beach_recommendations_bulk, see 20260609b)
-- now emits hour_score_v3; the live compute fn called by beach.html
-- (`beach.html:1329`) needs to emit score_v3 too so the chart drill can
-- read V3 once we flip beach.html to use it.
--
-- Status caps were already gone from this fn (per 20260606_drop_v2_caps).
-- Only Phase 1 change here: V3 path uses heat_uv_neg_raw + harsh_neg_raw
-- (no internal LEAST clamps). V2 path stays unchanged with its existing
-- LEAST(30) and LEAST(15) wrappers.
--
-- Added to JSONB output: 'score_v3' alongside existing 'score'. The
-- pos_* / neg_* drill stays as-is — those represent component values
-- (which V2 and V3 share for everything except heat_uv_neg / harsh_neg).
-- For drill display the V2 values are used; the chart's overall bar
-- height will use score_v3 once beach.html is flipped to read it.
...
begin
  ...
  with per_hour as (
    select h.local_hour, h.hour_label,
      ...
      -- V2 (existing): internally clamped at 30 / 15.
      least(30,
      ...
      -- V3 (Phase 1): raw band sums, no internal LEAST clamp.
      (
        public._v2_lookup_band(coalesce(h.sand_temp, -1), v_b_sand_neg)
        + public._v2_lookup_band(coalesce(h.asphalt_temp, -1), v_b_asphalt_neg)
        + public._v2_lookup_band(coalesce(h.uv_index, -1), v_b_uv_neg)
