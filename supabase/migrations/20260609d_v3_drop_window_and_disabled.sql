-- 20260609d — V3 drops window_pos + disabled_access
--
-- WHY: Franz 2026-06-09:
--   - window_pos is circular: best_window is picked using the score that
--     itself includes window_pos. Self-reinforcing, conceptually wrong.
--   - disabled_access is a FILTER (does this beach work for someone with
--     mobility needs), not a quality signal. Removing it from amenities
--     means a beach without it can still score as well as one with it.
--
-- These changes apply to V3 ONLY. V2 retains window_pos + disabled bonus
-- so the shadow comparison stays meaningful.
--
-- amenities_pos_v3 = sum WITHOUT disabled (max 17 vs V2's 18).
-- score_v3 formula = base + amenities_v3 + dog_access + weather - negs + drive.
--   (no + window_pos)
--
-- The pos_disabled + pos_window fields in the live fn's JSONB drill stay
-- so the chart can still show "this beach has disabled access" as a flag
-- — but it's display-only, not score-contributing in V3.
...
BEGIN
  ...
  SELECT
    ...
    -- V3 amenities: excludes disabled (per Franz 2026-06-09 — filter not score)
    g.v_restroom_pos + g.v_lifeguard_pos + g.v_parking_pos
      + g.v_shower_pos + g.v_picnic_pos + g.v_food_pos
  ...
  SELECT
    ...
    -- V2 score: unchanged. Includes window_pos and disabled bonus.
    ...
    END AS score,
    -- V3 score: no window_pos (circular), no disabled (it's a filter).
    -- Uses raw negs (no LEAST clamp) per Phase 1.
    CASE WHEN is_closed THEN 0
         ELSE greatest(0, least(100,
  ...
  -- Best window picking — still uses V2 score for stability.
...
begin
  ...
  -- V3: excludes disabled (Franz 2026-06-09 — it's a filter, not a score)
