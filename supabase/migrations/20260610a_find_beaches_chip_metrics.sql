-- 20260610a_find_beaches_chip_metrics.sql
--
-- Extend find_beaches RPC to return four new fields used by the
-- redesigned find.html beach cards (index.html parity):
--
--   avg_uv             — passthrough from beach_day_recommendations
--   peak_sand_temp     — MAX(sand_temp)    over the day's daylight hours
--   peak_asphalt_temp  — MAX(asphalt_temp) over the day's daylight hours
--   peak_wave_height_m — MAX(wave_height_m) from marine_grid_hourly over
--                        the day's daylight hours; NULL on inland beaches
--                        (no marine_grid coords on beaches_gold).
--
-- Cost shape: peaks aggregation joins beach_day_hourly_scores + marine
-- grid against the post-LIMIT `ranked` set (≤500 beaches). PK lookups on
-- both hourly tables are index-fast.
...
AS $function$
  ...
  -- ─── Surface caution peaks — sand_temp + asphalt_temp ────────────
  -- MAX over the day's daylight hours, per ranked beach. Chip is only
  -- rendered when the peak crosses an advisory band, so passing the raw
  -- peak (not pre-thresholded) lets the frontend pick the band label.
  peaks AS (
    SELECT
      h.arena_group_id,
  ...
  -- ─── Surf height — peak wave_height_m from marine_grid_hourly ────
  -- Aggregated over the day's daylight UTC window (≈ local_date ± 12h)
  -- for the cell each beach is bound to. NULL for inland beaches whose
  -- marine_grid_lat/lon are NULL.
  surf AS (
    SELECT
      r.arena_group_id,
