-- 20260610b_find_beaches_hero_photo.sql
--
-- Extend find_beaches RPC to return a per-beach hero thumbnail URL for
-- the redesigned find.html card thumbnail tile.
--
-- Why not call get_beach_photos_diverse(): plpgsql + per-call DROP/CREATE
-- of pg_temp._cand accumulates relation locks across ≤500 ranked
-- beaches per call. See [[get_beach_photos_diverse-temp-table-lock-leak]].
--
-- Instead: a SQL-native LATERAL pick that mirrors the diverse selector's
-- hero choice. Eligibility = curated OR keep_prob ≥ 0.65 (matches the
-- diverse selector's eligible CTE). Priority: has_dog DESC, curated
-- DESC, eff_score DESC, sort_order ASC. Lateral is index-fast via
-- beach_photos_active_idx (arena_group_id) WHERE hidden_at IS NULL.
...
AS $function$
  ...
  -- ─── Hero photo — SQL-native pick mirroring the diverse selector ─
  -- Priority: has_dog DESC, curated DESC, eff_score DESC, sort_order ASC.
  -- Lateral lookup is index-fast via beach_photos_active_idx.
  hero AS (
    SELECT
      r.arena_group_id,
