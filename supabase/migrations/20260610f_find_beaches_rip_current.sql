-- 20260610f_find_beaches_rip_current.sql
--
-- Extend find_beaches RPC: surface rip_current_risk from the NWS SRF data
-- path. Beaches without a bound coastal_zone_id (inland) or whose zone
-- doesn't have a current-period row return NULL → frontend suppresses
-- the chip.
...
AS $function$
  ...
  LEFT JOIN public.wfo_srf_forecast srf
         ON srf.zone_id = r.coastal_zone_id
        AND srf.forecast_date = p_date
