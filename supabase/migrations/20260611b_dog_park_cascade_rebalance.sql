-- 20260611b_dog_park_cascade_rebalance.sql
--
-- Port Item 2 of the beach Tier 1 fixes (20260611_v3_tier1_fixes.sql) to
-- dog parks per HARD rule [[paired-functions-port-fixes-both-sides]].
--
-- Dog-park heat_uv_neg has the same cascade structure as beaches:
-- atmospheric inputs (UV, feels_like) score DIRECTLY in heat_uv_neg AND
-- INDIRECTLY via asphalt_temp scoring (asphalt_temp is derived from
-- temp_air + UV + wind by v1's surface-temp physics). The direct UV
-- band and the feels_like>90 extra were piled on top of asphalt_neg —
-- same double-debit pattern Item 2 fixed for beaches.
--
-- Items 1 and 3 from the beach Tier 1 work do NOT port:
--   - Item 1 (hardcoded 6-21): dog parks already read
--     dog_park_dog_policy.hours_open_time/close_time with 6/22 NULL
--     fallback. Different code path, not the same bug.
--   - Item 3 (v3 picker): dog parks have no hour_score_v3 — the Phase 1
--     v3 reform was beach-only. No v3 score to switch the picker to.
--
-- Rebalance halves the two direct paths, keeping the indirect path
-- (asphalt_neg band) unchanged — same approach as beaches:
--   - uv_neg band:                     max 10 → max 5
--   - feels_like>90 extra multiplier:  *0.5  → *0.25
--   - feels_like>90 extra cap:         5     → 3
--
-- Net effect on max heat_uv_neg (dog parks):
--   pre:  asphalt 15 + uv 10 + feels 5 = 30 (clamped to 25)
--   post: asphalt 15 + uv 5  + feels 3 = 23 (no longer hits the 25 cap)

begin;

-- ── uv_neg band rebalance ────────────────────────────────────────────
-- Old: 0 (<6) → 1 (6-8) → 3 (8-10) → 4 (10-11) → 10 (≥11)   max 10
-- New: 0 (<6) → 1 (6-8) → 2 (8-10) → 3 (10-11) → 5 (≥11)    max 5
-- (Dog parks keep the 5-tier ceiling slightly above beach's 4 because
-- direct sun exposure on asphalt without surf moderation is harsher.)
update public.scoring_config_v2
   set bands = jsonb_build_array(
     ...
     jsonb_build_object('min', 11,            'score', 5)
   )
 where entity_type = 'dog_park' and signal_key = 'uv_neg';


-- ── compute_dog_park_hourly_v2 ───────────────────────────────────────
-- Only one expression changes: the feels_like > 90 extra inside
-- heat_uv_neg. All other logic is identical to current live state.
...
begin
  ...
  with per_hour as (
    select h.local_hour, h.hour_label,
      ...
      -- ITEM 2 PORT: feels_like > 90 extra halved (0.5 → 0.25, cap 5 → 3).
      -- asphalt_neg + uv_neg bands carry the heat half via the surface-temp
      -- physics; the direct path no longer compounds at full weight.
      least(25,
        ...
        + case when h.feels_like > 90
               then least(3, greatest(0, round((h.feels_like - 90) * 0.25)::int))
               else 0 end
      ) as heat_uv_neg,
      least(15,
