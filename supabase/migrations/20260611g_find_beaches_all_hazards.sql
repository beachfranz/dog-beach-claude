-- 20260611g_find_beaches_all_hazards.sql
--
-- Extend find_beaches RPC with a `hazards JSONB` column carrying all
-- active hazard advisories per beach (the 8 categories landed in
-- 20260611e). Sorted severe → moderate. Keeps the existing
-- rip_current_risk text column for backward compat with the existing
-- ripBump ranking logic on find.html.
--
-- Frontend chip rendering: find.html iterates b.hazards to emit a chip
-- per hazard. Severity 'severe' renders as no_go (red), 'moderate' as
-- caution (amber). Per-hazard icon + label mapping lives in the
-- frontend (no need to duplicate in SQL).
...
AS $function$
  ...
  -- ─── Active hazards per beach — JSONB array sorted severe→moderate ─
  -- Covers the 8 categories landed in 20260611e (BHS extraction + SRF
  -- merge). Frontend renders one chip per row, with per-hazard icon /
  -- label mapping client-side.
  hazards_agg AS (
    SELECT
