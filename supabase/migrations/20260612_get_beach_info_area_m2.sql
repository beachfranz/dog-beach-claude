-- 20260612_get_beach_info_area_m2.sql
--
-- Expose beaches_gold.area_m2 in the get_beach_info RPC's `beach`
-- jsonb so beach.html can render an area amenity tile mirroring the
-- one on dog-park.html (line 1281: "📐 0.X acres" from area_m2).
--
-- 2,110 of 3,942 scoreable beaches (53.5%) have area_m2 populated via
-- 20260606y_beach_osm_polygon_area.sql / refresh_beach_polygon_membership.
-- Beaches without it return area_m2: null and the frontend just doesn't
-- render the tile. No backfill in this migration.
--
-- One-field add. Otherwise identical to 20260607h_get_beach_info_date_param.sql.
...
AS $function$
  ...
  SELECT jsonb_build_object(
    'beach', jsonb_build_object(
      ...
      -- New 2026-06-12: surface beach polygon area so beach.html can
      -- render "📐 0.X acres" amenity tile (mirrors dog-park.html).
      'area_m2',       g.area_m2
      ),
