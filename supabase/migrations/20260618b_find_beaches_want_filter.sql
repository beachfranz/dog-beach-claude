-- find.html perf: push the leash ("want") filter server-side so the no-geo
-- pool can shrink from 500 -> ~50. find.html's matchesWant filters on
-- has_off_leash / has_on_leash (NOT access_rule), so p_leash is the wrong
-- column for this — add two dedicated boolean params instead.
--
--   p_want_off / p_want_on (default NULL):
--     both NULL  -> no want filter (existing callers e.g. get-beach-compare
--                   are byte-for-byte unaffected)
--     either set -> mirror matchesWant: a beach qualifies if it has the
--                   wanted zone(s). Mixed beaches (both zones) pass under
--                   either flag.
--
-- Body is otherwise identical to 20260615i_find_beaches_v3_cutover.sql.

BEGIN;

-- Drop the previous 7-arg signature; adding params creates a new overload,
-- and two coexisting find_beaches functions make PostgREST RPC calls ambiguous.
...
AS $function$
  ...
  base AS (
    ...
    WHERE g.is_active = true
      ...
      -- server-side want filter, mirrors find.html matchesWant (has_*_leash).
      AND (
        (p_want_off IS NULL AND p_want_on IS NULL)
