-- compute_beach_hourly_v2: v3 dog-access from zone_rules (sand 10/5 +
-- water 3/2 + trails 2/1) + new water_score (zone access 6/3 + swim-
-- comfort from marine temp/waves). v2 path byte-identical (keeps the
-- old off_leash_flag +5). Adds marine_grid_hourly join.

BEGIN;

CREATE OR REPLACE FUNCTION public.compute_beach_hourly_v2(p_fid bigint, p_user_lat double precision DEFAULT NULL::double precision, p_user_lng double precision DEFAULT NULL::double precision, p_date date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
AS $function$
declare
  ...
  v_dog_access_v3 integer := 0;
  v_water_access_base integer := 0;
...
begin
  ...
  v_amenities_pos_v3 := v_restroom_pos + v_lifeguard_pos + v_parking_pos
                     + v_shower_pos + v_picnic_pos + v_food_pos
                     + v_drinking_pos;
  if v_policy.off_leash_flag = true then v_dog_access_pos := 5; end if;  -- v2 (unchanged)
  -- v3: zone_rules-derived dog access (sand 10/5 + water 3/2 + trails 2/1)
  v_dog_access_v3 := public._dog_access_score_v3(
    v_policy.zone_rules, v_policy.off_leash_flag, v_policy.has_off_leash, v_policy.has_on_leash);
  v_water_access_base := public._water_access_base_v3(v_policy.zone_rules);
  ...
  with per_hour as (
    select h.local_hour, h.hour_label,
      ...
      m.water_temp_f                  AS water_temp_f,
      (m.wave_height_m * 3.28084)     AS wave_ft
    from public.beach_day_hourly_scores h
    left join public.marine_grid_hourly m
      on m.grid_lat = v_gold.marine_grid_lat
     and m.grid_lon = v_gold.marine_grid_lon
     and m.forecast_ts = (v_view_date::timestamp + (h.local_hour || ' hours')::interval) at time zone v_gold.timezone
  ...
  composed as (
    select *,
      ...
      v_dog_access_v3 as dog_access_v3,
      -- water_score = zone access base + swim-comfort (temp + waves); 0 when
      -- the beach has no dog water access.
      CASE WHEN v_water_access_base = 0 THEN 0
           ELSE v_water_access_base
                + CASE WHEN water_temp_f IS NULL THEN 0
                       WHEN water_temp_f BETWEEN 64 AND 75 THEN 3
                       WHEN water_temp_f BETWEEN 60 AND 80 THEN 1 ELSE 0 END
                + CASE WHEN wave_ft IS NULL THEN 0
                       WHEN wave_ft < 2 THEN 2
                       WHEN wave_ft <= 4 THEN 1 ELSE 0 END
      END AS water_score
    from per_hour
  ),
  scored as (
    select *,
      ...
      case
        ...
        when feels_like   <= 20    then 0
          ...
          50 + amenities_static_v3 + dog_access_v3 + weather_pos + water_score
  ...
  select jsonb_agg(jsonb_build_object(
    ...
    'pos_dog_access', case when is_closed then 0 else dog_access_v3 end,
    'pos_water', case when is_closed then 0 else water_score end,
    'water_temp_f', water_temp_f,
    'wave_ft', case when wave_ft is not null then round(wave_ft::numeric, 1) else null end,
