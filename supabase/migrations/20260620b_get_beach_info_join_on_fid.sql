-- 20260620b_get_beach_info_join_on_fid.sql
--
-- Fix: get_beach_info (beach.html's RPC) joined beach_day_recommendations,
-- beach_day_hourly_scores, beach_dog_policy, and beach_amenities on
-- g.group_id, but those tables key arena_group_id to the FID per HARD rule
-- [[arena_group_id-is-fid-not-group]]. The sibling find_beaches RPC (and the
-- beach_descriptions join in this very function) correctly use g.fid.
--
-- Effect of the bug: for the 513 of 4,024 scored beaches where
-- fid <> group_id, beach.html showed a blank score (group_id with no rows,
-- e.g. Paradise Cove 623/9364) OR an entirely different beach's score, dog
-- policy, and amenities (group_id that collides with another beach's fid,
-- e.g. Hamilton 747/8404). find.html was always correct (joins on fid).
--
-- Fix: repoint the four g.group_id joins to g.fid. CREATE OR REPLACE, same
-- signature → no edge-function redeploy needed.
...
AS $function$
  ...
  SELECT jsonb_build_object(
    ...
    'hours_today', coalesce((
      ...
      WHERE h.arena_group_id = g.fid AND h.local_date = d.target_date
    ), '[]'::jsonb),
    'photos', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
        'source',      sp.source,
  ...
  LEFT JOIN public.beach_day_recommendations dr
    ON dr.arena_group_id = g.fid AND dr.local_date = d.target_date
