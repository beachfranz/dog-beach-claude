-- 20260620c_find_beaches_text_search.sql
--
-- Wire up the find.html search box: add a free-text p_query param to
-- find_beaches. When set, it searches the FULL catalog by name (not just the
-- scored subset) and OVERRIDES the leash / want / scored filter gates, so a
-- name match always surfaces. match_rank orders exact > prefix > substring >
-- county/state, then the existing distance/score tiebreak.
--
-- Body is otherwise identical to 20260618b_find_beaches_want_filter.sql.
-- Adding a param creates a new overload, so DROP the 9-arg signature first
-- (two coexisting find_beaches make PostgREST RPC calls ambiguous).

BEGIN;

DROP FUNCTION IF EXISTS public.find_beaches(
  date, double precision, double precision, text, integer, boolean, boolean, boolean, integer
);
