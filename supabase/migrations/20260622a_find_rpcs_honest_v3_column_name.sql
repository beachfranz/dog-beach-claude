-- 20260622a_find_rpcs_honest_v3_column_name.sql
--
-- "Fix the lie": the find_beaches / find_dog_parks RPCs RETURN a column named
-- `composite_score_v2` whose CONTENT has been COALESCE(composite_score_v3,
-- composite_score_v2) since the 2026-06-15 v3 cutover (20260615i/j). The name
-- lied. Rename the output column to `composite_score_v3` so it matches the
-- v3-first content.
--
-- Renaming a RETURNS TABLE output column requires DROP + CREATE (CREATE OR
-- REPLACE cannot change OUT-parameter names).
--
-- Scope of this rename is the OUTPUT COLUMN NAME only. The underlying table
-- columns beach_day_recommendations.composite_score_v2 / _v3 (real v2 / v3
-- data) are untouched, as is nearest_dog_parks.composite_score_v2 (which holds
-- genuine v2 and is therefore honest) and find_dog_parks.composite_score (a
-- separate go-hours-percentage column).
--
-- Consumer updates land alongside this migration:
--   * find.html normalizeBeach reads b.composite_score_v3
--   * get-beach-compare output field renamed (edge fn redeploy)
-- find_dog_parks's only caller (get-dog-parks-find) is a pass-through with no
-- HTML consumer, so no edge-fn change is needed there.

BEGIN;

-- ───────────────────────── find_beaches ─────────────────────────
-- Body identical to 20260620c_find_beaches_text_search.sql; only the
-- RETURNS TABLE column `composite_score_v2` and its final-SELECT alias are
-- renamed to `composite_score_v3`.

DROP FUNCTION IF EXISTS public.find_beaches(
  date, double precision, double precision, text, integer, boolean, boolean, boolean, integer, text
);
...
CREATE FUNCTION public.find_beaches(
  ...
  p_want_off boolean DEFAULT NULL,
  p_want_on boolean DEFAULT NULL,
...
RETURNS TABLE(
  ...
  composite_score_v3 integer, current_hour_score integer,
...
AS $function$
  ...
  base AS (
    SELECT
      ...
      CASE
        ...
        ELSE NULL
      END AS distance_m,
      -- text-search match rank (NULL when not searching). exact > prefix >
      -- substring on display name, then county/state substring.
      CASE
        WHEN p_query IS NULL THEN NULL
        WHEN lower(coalesce(g.display_name_override, g.name)) = lower(p_query) THEN 0
        WHEN lower(coalesce(g.display_name_override, g.name)) LIKE lower(p_query) || '%' THEN 1
        WHEN coalesce(g.display_name_override, g.name) ILIKE '%' || p_query || '%' THEN 2
        ELSE 3
      END AS match_rank,
    ...
    WHERE g.is_active = true
      -- text search: match name / county / state. Overrides the filter
      -- gates below so a named beach always surfaces.
      AND (
        p_query IS NULL
        OR coalesce(g.display_name_override, g.name) ILIKE '%' || p_query || '%'
        OR g.county_name ILIKE '%' || p_query || '%'
        OR g.state ILIKE p_query
      )
      -- leash gate (bypassed in search mode)
      AND (p_query IS NOT NULL OR p_leash = 'any' OR dp.access_rule = p_leash OR dp.access_rule IS NULL)
      -- scored gate (bypassed in search mode); v3 cutover accepts either column.
      AND (p_query IS NOT NULL OR NOT p_scored_only OR COALESCE(dr.composite_score_v3, dr.composite_score_v2) IS NOT NULL)
      -- server-side want filter (bypassed in search mode), mirrors find.html matchesWant.
      AND (
        p_query IS NOT NULL
        OR (p_want_off IS NULL AND p_want_on IS NULL)
        OR (COALESCE(p_want_off, false) AND dp.has_off_leash)
        OR (COALESCE(p_want_on,  false) AND dp.has_on_leash)
      )
  ),
  ...
  ranked AS (
    ...
    FROM base b
    ORDER BY
      -- text search: best name match first.
      CASE WHEN p_query IS NOT NULL THEN b.match_rank ELSE NULL END ASC NULLS LAST,
      CASE
        WHEN p_lat IS NOT NULL AND p_lng IS NOT NULL
  ...
  SELECT
    ...
    r.composite_score AS composite_score_v3,
  ...
  -- preserve ranked ordering (name-match first in search mode)
  ORDER BY
    ...
    CASE
      ...
      ELSE NULL
    END NULLS LAST,
    CASE WHEN p_lat IS NULL AND p_lng IS NULL THEN r.composite_score ELSE NULL END DESC NULLS LAST;
$function$;

-- ───────────────────────── find_dog_parks ─────────────────────────
-- Body identical to 20260615j_find_dog_parks_v3_cutover.sql; only the
-- RETURNS TABLE column `composite_score_v2` and its SELECT alias are renamed
-- to `composite_score_v3`. The separate trailing `composite_score numeric`
-- (go-hours percentage) is left as-is.

DROP FUNCTION IF EXISTS public.find_dog_parks(
  date, double precision, double precision, text, integer, boolean, text, boolean, boolean
);
...
RETURNS TABLE(
  ...
  composite_score_v3 integer, best_window_label text, best_window_status text,
...
AS $function$
  SELECT
    ...
    COALESCE(dr.composite_score_v3::int, dr.composite_score_v2) AS composite_score_v3,
